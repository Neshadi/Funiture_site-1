...
const UserLoginPopUp = ({ setShowLogin, setUserType, setIsLoggedIn }) => {
  ...
  const signUp = async (e) => {
    ...
    try {
      ...
      setError(
        err.response?.data?.error || "Error during sign-up. Please try again."
      );
    }
  };
  ...
  const logIn = async (e) => {
    ...
    try {
      const result = await axios.post(
        "https://new-sever.vercel.app/api/users/auth",
        {
          email: email,
          password: password,
        },
  ...
  const adminLogIn = async (e) => {
    ...
    try {
      const result = await axios.post(
        "https://new-sever.vercel.app/api/users/adminauth",
        {
          email: email,
          password: password,
        },
  ...
  return (
    <div className="login-popup">
      <form onSubmit={checkSubmission} className="login-popup-container">
        ...
        <button id="button3" onClick={fogotPassowrdHandler}>
          Fogot Password
        </button>
